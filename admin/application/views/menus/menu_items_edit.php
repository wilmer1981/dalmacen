 ...
 <div class="page-content">
    <div class="page-header">
        <h1>Menus
            <small><i class="ace-icon fa fa-angle-double-right"></i> Editar Item</small>
        </h1>
			...
			<div  class="row">	
				<div  class="col-md-8">	
					<div class="form-group">
						...
						<div class="controls">
						  <input id="txtTitulo" type="text" name="txtTitulo" value="<?php echo $registro[0]->titulo; ?>" class="form-control" placeholder="Ingrese Titulo" required=""  autofocus="">
						</div>
					</div>												
				</div>
				<div  class="col-md-4">					
					<div class="form-group">				
						...
						<div class="controls">									
							  <input id="txtAlias" type="text" name="txtAlias" value="<?php echo $registro[0]->alias; ?>" class="form-control" placeholder="Autogenerado desde el título" readonly >
						</div>
					</div>
				</div>						
			</div>
										...
										<div class="col-md-12">
											...
											<div class="form-group">
											   <label for="inputMarca">Link :<span class="required"></span></label>
											    <div class="controls">									
													<input id="txtLink" type="text" name="txtLink" class="form-control" value="<?php echo $registro[0]->link; ?>">
												</div>
										    </div>											
									  </div> 
								...
								<div class="col-md-3">
										<div class="row">
										  ...
										  <div class="form-group required">
											  <label for="inputMarca">Menu:</label>
												<div class="controls">	
												<select id="cboMenu" name="cboMenu" class="form-control" required>
												<option value="">-- Seleccione --</option>
												<?php foreach ($tipomenu as $t) {
														  $documento = mb_strtoupper($t->titulo, 'UTF-8');													
														 if($t->id == $registro[0]->id_tipo){ 
															  $selected = 'selected'; 														
														  }else{	
															 $selected = ''; 														
														  }  
											...
											<div class="form-group">
											  <label for="inputMarca">Elemento Padre:<span class="required"></span></label>
												...
												<?php foreach ($menusitem as $t) {
														//$documento = mb_strtoupper($t->documento, 'UTF-8');
														$documento = $t->titulo;													
														echo '<option value="'.$t->id.'"> - '.$documento.'</option>';
														/*
														if(isset($usuarios[0]->id_departamento)){
																$result = getCboCargo($usuarios[0]->id_departamento);
																foreach ($result as $t) {
																	$titulo = mb_strtoupper($t->titulo, 'UTF-8');				
																	if($t->id == $usuarios[0]->id_cargo){ 
																		$selected = 'selected';							
																	}else{
																		$selected = '';									
																	}												  
																echo '<option value="'.$t->id.'"'.$selected.'>'.$titulo.'</option>';
																}
														}else{
																foreach ($cargos as $t) {
																	$titulo = mb_strtoupper($t->titulo, 'UTF-8');				
