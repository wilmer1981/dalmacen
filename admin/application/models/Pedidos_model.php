...
*/
  ...
  public function getPedidos($where='',$one=false,$array='array'){             
      $this->db->select('p.*,
	 ...
	 // $this->db->join('wsoft_productos y', 'y.id = d.id_producto', 'inner');
      $this->db->join('wsoft_clientes c', 'c.id = p.id_cliente', 'inner');
     // $this->db->join('wsoft_documentos d', 'd.id = p.id_tipocomprobante', 'inner');
     // $this->db->join('traslados t', 't.id_pedido = p.id', 'left');
      //$this->db->join('ventas v', 'v.id_pedido = p.id', 'left');
      //$this->db->limit($perpage,$start);
      if($where){
          $this->db->where($where);
      }
	  $this->db->group_by("d.id_pedido");

	  //$this->db->group_by('p.id_cliente');
    ...
    public function getPedidosDetalle($where='',$one=false,$array='array'){             
      $this->db->select('p.*,d.cantidad,d.precio_unit, y.nombre as producto,
                       IF(c.id_grupo = "2", "", CONCAT(c.nombres," ",c.apellidos)) as cliente					 
					   ',				 
					  FALSE);
      $this->db->from('wsoft_pedidos p');
      $this->db->join('wsoft_pedidos_detalle d', 'd.id_pedido = p.id', 'inner');
  ...
  public function get($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array'){      
	  ...
	  public function getById($id){
		  $this->db->select('p.*, p.id_unidad, s.nombre as subcategoria, c.id as id_categoria,c.nombre as categoria, d.id_marca, d.id_modelo, m.nombre as marca, n.nombre as modelo');
		  ...
		  $this->db->join('modelos n', 'n.id = d.id_modelo', 'left');
      ...
      //notificaciones para justificar: JEFE -- CONSULTOR
    //public function getNotificacionesPedido($tipo,$idemp,$fini,$ffin,$one=false,$array='array')
	public function getNotificacionesPedido($estado,$tipo,$one=false,$array='array')
    {
		...
		*/
        ...
        //if ($fini != "" && $ffin != ""){       
          //  $where = "DATE_FORMAT(r.fecha_reg,'%Y-%m-%d') BETWEEN '$fini' AND '$ffin'".$where;  
        //}	
		if($tipo=='total'){
			 $where="p.estado='".$estado."'";  
			$this->db->select('COUNT(DISTINCT p.id) as cantidad');
			$this->db->from('wsoft_pedidos_detalle d');
			$this->db->join('wsoft_pedidos p', 'p.id = d.id_pedido', 'inner');   
			//$this->db->group_by('r.fecha_reg');  
		}
		if($tipo=='lista'){
			$where="p.estado='".$estado."'";  
			$this->db->select('p.*, c.nombres, c.apellidos, c.url_imagen');
			$this->db->from('wsoft_pedidos p');		
			$this->db->join('wsoft_clientes c', 'c.id = p.id_cliente', 'inner'); 			
			//$this->db->group_by('r.fecha_reg');  
		}
