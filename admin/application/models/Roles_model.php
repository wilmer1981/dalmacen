...
*/
  ...
  public function count_all()
  {
	...
	public function getRoles($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array'){
        $this->db->select('*');      
		$this->db->from('usuarios_tipo');    
        $this->db->limit($perpage,$start);
        if($where){
            $this->db->where($where);
        }
	...
	public function getMenusTipo($one=false,$array='array'){
   		$sql = "SELECT t.id, t.titulo, t.estado,
      				COUNT(IF(m.estado = '1',1,NULL)) as publicados,
       				COUNT(IF(m.estado = '0',1,NULL)) as despublicados
				FROM menus_tipo AS t
				LEFT JOIN menus AS m ON m.id_tipo=t.id
	...
	public function getMenuItems($table,$fields,$where,$perpage=0,$start=0,$one=false,$array='array'){
          /*   
        $this->db->select('m.*, COUNT(IF(i.estado = "1",1,NULL)) as publicados, COUNT(IF(i.estado = "0",1,NULL)) as despublicados');
        $this->db->from('menus m');   
		$this->db->join('menus_items i', 'i.id_menu = m.id', 'left');		
        $this->db->limit($perpage,$start);
        if($where){
            $this->db->where($where);
        }     
		$this->db->group_by('i.id_menu');		
        $query = $this->db->get();
		*/
		$sql = "SELECT m.id, m.titulo, m.estado,
      				COUNT(IF(i.estado = '1',1,NULL)) as publicados,
       				COUNT(IF(i.estado = '0',1,NULL)) as despublicados
				FROM menus AS m
				LEFT JOIN menus_items AS i ON i.id_menu=m.id
				WHERE m.id_tipo=$where
	...
	public function getMenuTipoId($id){
